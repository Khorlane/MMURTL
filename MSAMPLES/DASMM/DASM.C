/*
 * Assembler for MMURTL (MMURTL Version).
 *
 * Copyright 1991,1992,1993,1994,1995 R.A. Burgess
  ...
  Version 1.7M 1/1/95 - V1.6 ported to MMURTL. Changed memory allocation
  				use AllocPage and also created internal buffers
  				instead of using tmp files. Also use an internal
  				read buffer of 4K to make better use of Readbytes().
				OutWord, OutDWord, and OutByte had to be renamed
				so they would not conflict with MMURTL publics of
				the same names.
...
#define LEVELS 5
U32 level = 0;				/* current include level */
...
#define STMPBUFS 512000

U8 *pcsbuf, *pdsbuf;	/* half meg allocated buffers for code & data */
U32 ics, ids;			/* current index into tmp code & data buffers */
...
S32  CBString;		/* size of parsed token */
S32  TSymnum;		/* Symbol table entry number, else 0 */
U32  TNumber;		/* value of numeric token */
S32  TInst;			/* Instruction number, else 0 */
S32  TReg;			/* Register number, else 0 */
S32  Token;			/* Token type (also returned from parse) */
...
S32 UCBString;		/* Size of Unknown label */
...
#define SYMSMAX 700
#define SYMBUFMAX 16384 	/* Avg of 10 byte per name, 700 Max */

struct symtab {
	U32  Type;		/* Token (e.g, CLabel) */
	U32  Line;		/* Line symbol was declared on */
	U32  Size;		/* Size of symbol name */
...
S32  iSymNext = 1;	/* index to next new symbol table entry.  */
...
#define LSYMBUFMAX 32768	  		/* Avg of 7 bytes per name */
...
S32  iLSymNext = 1;	/* index to next new symbol table entry.  */
					/* Starts at 1 - zero it reserved because */
					/* Parse returns 0 for EOL */
...
struct forreftab {
	...
	U32  Line;		/* Line reference was made on  (for error if not found) */
	S8   *Ptr;		/* Pointer to name in packed buffer */
...
S32  iRefNext = 0;	/* index to next new forward ref table entry.  */
...
struct extreftab {
	...
	U32  iSym;	 /* Index of gst entry. Hi bit set if CSeg ref */
	U32  Offs;	 /* Offset in segment were ref needs 32 bit fixing */
	};

struct extreftab ert[FREFSMAX];

S32  iERefNext = 0; 	/* index to next external ref table entry.  */
...
struct futab 
	...
	U32  iSym;	 /* index of DLL entry in gst, else 0 */
	};

struct futab *pfut;	/* pointer to allocated Fix Up Table */

S32  iFUNext = 0; 	/* index to next fix up table entry.  */
					/* Starts at 0 */
...
S32  rgToken[3][20]; /* Raw tokens in the operand (as many as 20) */
S32  rgVal[3][20];   /* value if token is number/displacement */
S32  rgTID[3][20];	 /* Register ID if token is a register */
S32  rgTCnt[3];		 /* total count of tokens in raw operand */
...
*/
S32  rgOpType[3];	/* Operand type for compare to instruction */
S32  rgOpReg[3];    /* If reg only, this which one */
...
S32  iInstEntry;	/* Which entry is it in rgINS */
S32  CrntInst;		/* instruction we are working on 	*/
S32  InstPfx;		/* Special Insturction prefix for crnt inst */
S32  nOperands;		/* number of operands we found */
...
S32  OpBase;        /* Base register if fBase is true */
S32  OpIndx;        /* Index register if fIndx is true */
S32  OpDisp;        /* Displacement if fDisp8 or fDisp32 is TRUE */
S32  iMemEntry;		/* If mem operand, which entry in rgM32 */
...
U8  ExpType0;
S32 nExpSyms;		/* Used by the numeric expression evaluator */
S32 iExpSym, iExpSym0;	/* if a symbol value translates to a SYMOFF,
...
void OutDWordX(unsigned long Data)
{
...
/******************************************************************/
/* Write a DWord to the code segment and DO NOT update counter */
/******************************************************************/
...
/******************************************************************/
/* Read n bytes from the code segment							 */
/******************************************************************/

void readCS(U8 *pDataRet, long cbData)
{
	while (cbData--)
		*pDataRet++ = pcsbuf[ics++];
}
...
/******************************************************************/
/* Write n bytes to the code segment and DO NOT update counter */
/******************************************************************/

void writeCS(U8 *pData, long cbData)
{
	while (cbData--)
		pcsbuf[ics++] = *pData++;
}
...
/****************************************************************
  Set index into code buffer
*****************************************************************/

void seekCS(long offset)
{
	ics = offset;
}
...
void readDS(U8 *pDataRet, long cbData)
{
	while (cbData--)
		*pDataRet++ = pdsbuf[ids++];
}
...
/******************************************************************/
/* Write n bytes to the data segment and DO NOT update counter */
/******************************************************************/

void writeDS(U8 *pData, long cbData)
{
	while (cbData--)
		pdsbuf[ids++] = *pData++;
}
...
/****************************************************************
  Set index into data buffer
*****************************************************************/

void seekDS(long offset)
{
	ids = offset;
}
...
void OutWordX(unsigned long Data)
{
unsigned char *pOut, b;
long i;
	pOut = &Data;
...
void OutByteX(unsigned char Data)
{
	if (fDataSeg) {
		pdsbuf[ids++] = Data;	/* fputc(Data , ds_fh); */
	    oNextData++;
	}
	else {
		pcsbuf[ics++] = Data;	/* fputc(b , cs_fh); */
	    oNextCode++;
	}
}
...
S32 findref(S8 *pb, S32 cb)  /* pointer to, and size of string */
{
S32 i;
S8   name[132];
...
S32  rgToken[3][20];  Raw tokens in the operand
S32  rgVal[3][20];    value if token is number/displacement
S32  rgTID[3][20];	  Register ID if token is a register
S32  rgTCnt[3];		  total count of tokens in raw operand
...
void FixUp(U8 typef, S32 Offset, S32 iSymbol)
{
...
void ExtRef(U8 EType, S32 iSymbol)
{
...
S32 INSEntry(S32 InsNum, S32 nOpers)
{
S32 i;
...
S8   sbyte;
S32  sword;
U32  i;		/* used local to each section if needed */
...
S32 AddFix;
	...
	ics = oNextCode - CodeOffset;
	ids = oNextData - DataOffset;
...
long sdata;
	...
	sdata = tag.len;                	/* WRITE the Code seg */
	fwrite(pcsbuf, 1, sdata, run_fh);

	tag.id = DATATAG;					/* Data Segment */
	tag.len = oNextData - DataOffset;	/* Size of DS.TMP */
	fwrite(&tag, 5, 1, run_fh);			/* Write the tag record */

	sdata = tag.len;                	/* WRITE the Data seg */
	fwrite(pdsbuf, 1, sdata, run_fh);
...
if (!(fgets(line_ptr = line_buf0, 100, src_fh[level])))
{
	...
	{		/* We are at the end of the ATF file */
...
if (fListA)
	...
	erc = AllocPage(LSYMBUFMAX/4096, &pLSymBuf);
	if (erc)
		fatal_error("Can't Allocate buffer 1\n");

	erc = AllocPage(SYMBUFMAX/4096, &pSymBuf);
	if (erc)
		fatal_error("Can't Allocate buffer 2\n");

	erc = AllocPage(MACBUFMAX/4096, &pMacBuf);
	if (erc)
		fatal_error("Can't Allocate buffer 3\n");

	erc = AllocPage(FREFBUFMAX/4096, &pRefBuf);
	if (erc)
		fatal_error("Can't Allocate buffer 4\n");

	erc = AllocPage(FREFTABMAX/4096, &pfrt);
	if (erc)
		fatal_error("Can't Allocate buffer 5\n");

	erc = AllocPage(FIXUPBUFMAX/4096, &pfut);
	if (erc)
...
erc = AllocPage(STMPBUFS/4096, &pcsbuf);
if (erc)
	fatal_error("Can't Allocate CS buffer \n");

erc = AllocPage(STMPBUFS/4096, &pdsbuf);
if (erc)
	fatal_error("Can't Allocate DS buffer \n");

ics = 0; 			/* current index into tmp buffers */
ids = 0;
