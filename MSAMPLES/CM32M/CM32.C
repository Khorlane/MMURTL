/*
 C Minus 32 (C-32) Compiler  (DOS version)

 Derived from code Copyright (c) 1989, Dave Dunfield
 Copyright (c) 1991, 1992, 1993, 1994, 1995 R.A. Burgess
...
/* These Simple defines to keep down the number of keystrokes. Also
   lessens that horrible burden of portability that C is supposed to
   shield you from (right...)
*/
...
#define SCODEBUF 512000		/* 500Kb allocated buffer */
U8 *pcodebuf;
...
struct sym {
	...
	U32 itypedef;	/* index in symtab for type definition, or zero */
					/* If strucmem, struct it belongs to */
	U32 oname;		/* offset in pool to sym name */
	U32 argoffset;	/* arg offset from EBP for locals */
	U32 strucoff;   /* size of strucdef (also offset for next new member), */
					/*	 if strucmem, it's member's offset in structure */
	U32	dindex;		/* rg for index to dim */
	};				/* or index to proto list for arg types */
					/* or holds labels for for local Goto */

struct sym symtab[MAX_SYMBOLS];  /* SYM TABLE... */

U32 oNextGName = 0;		/* offset to next new name in global pool */
U32 oNextLName = 0;		/* offset to next new name in local pool */
...
U32 proto_list[MAX_PROTOS],	/* list of arg types for functions */
    ...
    U32 CrntStrucDef;	/* iSym for Structure def we are using or working on */
...
U8 databuf[4096];
U32 pc = 0;			/* code pointer in */
U32 pco = 0;		/* code pointer out */
U32 pd = 0;
...
/******************************************
* Test for valid character in a name.(MACRO)
*******************************************/
...
{
	while((*buffin_ptr < 0x21) && (*buffin_ptr))
		++buffin_ptr;
	return *buffin_ptr;
}
...
* Return:	0	= No match
*           n	= Full match begining at peep_top
*				  and ending at entry 'n'
********************************************************/

static long compareT(char *ptable, long peep)
{
	int i;
	char *ptr1, *ptr2, c;
...
static void exchange(unsigned long old, char *pnew)
{
	char *ptr1, *ptr2;

    peep_top = (old+(OBUF_SIZE-1)) % OBUF_SIZE;  /* last entry of replacement */
	ptr2 = peep_buffer[peep_top];
	while(*pnew) {						/* while still some new stuff left */
		if(*pnew & 0x80) {				/* output a symbol */
			...
			*ptr2 = 0;					/* put null at end of line */
            peep_top = (peep_top+(OBUF_SIZE-1)) % OBUF_SIZE;
			ptr2 = peep_buffer[peep_top]; }
		else
			*ptr2++ = *pnew;
		++pnew; }
...
/***********************************************
   Simulates fgets from the HUGE code buffer
************************************************/
...
long n;
	...
	if (pco >= pc)		/* end of valid buffer data */
		return(0);

	while ((n < maxout-1) && (pcodebuf[pco]))
	{
		c = pcodebuf[pco];
		*pout++  = pcodebuf[pco++];
		n++;
...
char c, *sptr;
	...
	if(fgetcode(peep_buffer[peep_next], OLINE_SIZE)) {
...
{
	if (fGen)
		fputs(peep_buffer[peep_top], code_fh);
	else
		fputs(peep_buffer[peep_top], asm_fh);
	peep_top = (peep_top + 1) % OBUF_SIZE;
	if (fGen)
		fputs("\n", code_fh);
	else
		fputs("\n", asm_fh);
...
static void optimize(void)
{
	...
	for(;;) {
			/* keep buffer full! */
		while ( ((peep_next+1) % OBUF_SIZE) != peep_top ) 
		{
  	    	if (!read_line())
  	    	{
  	      		break;
...
static void UTC_error(void)
{
	...
	fputc(' ', list_fh);
	fputs("Unterminated comment from line: ", list_fh);
...
/* Generate all externals referenced from CSEG if NOT for DASM */
...
/* Generate all global variables that are not initialized */
    ...
    if (pd)
    {
		fwrite(databuf, pd, 1, asm_fh);
		pd = 0;
	}

	if (!error_count)
	{  /* No optimize if errors */

		if (fNoOpt) {

				while (fgetcode(buffer, LINE_MAX))
				{
					if (fGen)
...
{
	S32 ndim, vcnt;
	U32 i, j, size, token, t, v, tp, tp1, ofs;
...
/********************************
* Write a number to the code buf
*********************************/

void code_num(U32 value)
{
	char stack[10];
	U32 i;
...
void data_chr(char chr)
{
    databuf[pd++] = chr;
    if (pd > 4095)
    {
		fwrite(databuf, 4096, 1, asm_fh);
		pd = 0;
	}
}
...
/********************************
* Write a number to the data buf
*********************************/

void data_num(U32 value)
{
	char stack[10];
	U32 i;
...
void put_num(U32 value, FILE *outfile)
{
	char stack[10];
	U32 i;
...
static void statement(U32 token)
{
	...
	{		/* act upon the token */
		...
		case CASE:
			a = check_switch();
			get_constant(&b, &c);
			switch_stack[switch_ptr++] = a;  /* RAB reversed a & c */
...
/* first, get a BIG code buffer!! */

	erc = AllocPage(SCODEBUF/4096, &pcodebuf);
	if (erc)
...
/* Output file not explicitly named defaults to Source.asm */
	...
	if(!asm_fh)	
	{					/* default asm name to SourceName.asm */
